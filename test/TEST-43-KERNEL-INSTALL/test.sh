...
test_setup() {
    ...
    echo 'uki_generator=none' >> /etc/kernel/install.conf
    ...
    if command -v kernel-install > /dev/null; then
        ...
        kernel-install add "$KVERSION" "$VMLINUZ"
    else
        echo 'layout=compat' >> /etc/kernel/install.conf
        installkernel "$KVERSION" "$VMLINUZ"
    fi
}

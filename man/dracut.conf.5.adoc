...
*prefer_busybox=*"__{yes|no}__"::
If the busybox module is included, do not copy host binaries into the
initrd when busybox provides an equivalent applet. This reduces initrd
size at the cost of busybox's reduced feature set (default=no).

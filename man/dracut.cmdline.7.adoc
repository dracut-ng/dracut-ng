...
device already contains a valid LUKS volume and a password is provided, it will
be opened as-is.  Otherwise, the device will be wiped, formatted with LUKS, and
a new filesystem will be created on it.  If a password is not provided, a random
one will be generated and stored in `/run/initramfs/overlayfs.passwd`.
+
...
* _timeout=<seconds>_ - Time to wait for the device to appear (default: 0,
  meaning don't wait).
--
+
[listing]
...
rd.overlay.crypt=/dev/sdb,pass=mysecret

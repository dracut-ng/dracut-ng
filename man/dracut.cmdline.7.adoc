...
**rd.overlay.crypt=**__<options>__::
Requires the dracut 'overlayfs-crypt' module.
+
Specifies an encrypted (LUKS) device for persistent overlay storage.  The device
will be formatted with LUKS encryption and a filesystem will be created on it.
If a password is not provided, a random one will be generated and stored in
`/run/initramfs/overlayfs.passwd`.
+
Supported options (comma-separated):
+
--
* _dev=<device>_ - (Required) Device specification: LABEL=, UUID=, PARTLABEL=,
  PARTUUID=, /dev/<device>, or short device name.
* _pass=<password>_ - Encryption password.  If omitted, a random password is
  generated.
...
* _timeout=<seconds>_ - Time to wait for device to appear (default: 0).
--
+
[listing]
.Examples
----
rd.overlay.crypt=dev=LABEL=CRYPTOVL
rd.overlay.crypt=dev=/dev/sdb,pass=mysecret
rd.overlay.crypt=dev=UUID=12345678-1234-1234-1234-123456789abc,fstype=ext3
----

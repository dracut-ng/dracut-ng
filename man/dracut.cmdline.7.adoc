...
+
NOTE:
    For live boot (root=live:...), the overlayroot= parameter works equivalently
    to rd.overlay=. Both parameters support the same syntax (LABEL=, UUID=,
    etc.) and share auto-partitioning functionality when LABEL= is specified but

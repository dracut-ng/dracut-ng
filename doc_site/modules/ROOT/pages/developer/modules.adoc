...
Anything in the 80-89 range is stuff that dracut relies on,
so try not to break those hooks.
...
dracut modules. Range 00-09 and 90-99 are reserved for unusual
dracut customizations for completeness.

Not using the 50-59 range for out of tree dracut modules
will likely lead to unintended erros in the initramfs
